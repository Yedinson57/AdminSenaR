...
export default function ComputerShow() {
  ...
  useEffect(() => {
    setItem({
      id,
      number: 12, brand: 'Lenovo', environment_id: 1, urlFoto: 'https://via.placeholder.com/600x400',
  ...
  return (
    <RequireAuth>
      <FormCard title="Información del Equipo" subtitle={item.name || item.title || item.code || `Registro #${item.id}`}>
        
        <div className="mb-4">
          <span className="text-muted small d-block mb-1 fw-bold text-uppercase">Número</span>
          <div className="p-3 bg-light rounded-3 fw-medium text-dark">{item.number}</div>
        </div>
        <div className="mb-4">
          <span className="text-muted small d-block mb-1 fw-bold text-uppercase">Marca</span>
          <div className="p-3 bg-light rounded-3 fw-medium text-dark">{item.brand}</div>
        </div>
        <div className="mb-4">
        ...
        <div className="d-flex justify-content-center mt-4">
          <button type="button" onClick={() => navigate('/computer')} className="btn btn-dark px-4 py-2 fw-bold rounded-3">
            Volver al Listado
          </button>
        </div>
      </FormCard>
