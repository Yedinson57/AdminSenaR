...
export default function TeacherEdit() {
  ...
  const [formData, setFormData] = useState({name:"",email:"",area_id:"",training_center_id:"",urlFoto:null});

  useEffect(() => {
    setFormData((prev) => ({ ...prev, name: prev.name || 'Registro teacher #' + id }));
  }, [id]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log('Actualizar teacher', id, formData);
    navigate('/teacher');
  };

  return (
    <RequireAuth>
      <FormCard title="Actualizar Instructor" subtitle="Modifique los datos del registro seleccionado">
        <form onSubmit={handleSubmit} encType="multipart/form-data">
                  ...
                  <div className="form-floating mb-4">
                    <input type="email" className="form-control" id="email" name="email" placeholder="Correo Electrónico Institucional" value={formData.email} onChange={handleChange} required />
                  ...
                  <div className="form-floating mb-4">
                    ...
                    <label htmlFor="training_center_id" className="text-secondary">Centro de Formación Asignado</label>
                  </div>

                  <div className="mb-4">
                    <label htmlFor="urlFoto" className="form-label fw-bold text-secondary mb-1">Fotografía del Instructor</label>
