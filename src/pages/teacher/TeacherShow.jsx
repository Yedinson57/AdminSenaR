...
export default function TeacherShow() {
  ...
  useEffect(() => {
    setItem({
      id,
      name: 'Carlos Pérez', email: 'cperez@sena.edu.co', area_id: 1, training_center_id: 1, urlFoto: 'https://via.placeholder.com/600x400',
  ...
  return (
    <RequireAuth>
      <FormCard title="Detalle del Instructor" subtitle={item.name || item.title || item.code || `Registro #${item.id}`}>
        
        <div className="mb-4">
          <span className="text-muted small d-block mb-1 fw-bold text-uppercase">Nombre Completo</span>
          <div className="p-3 bg-light rounded-3 fw-medium text-dark">{item.name}</div>
        </div>
        <div className="mb-4">
          <span className="text-muted small d-block mb-1 fw-bold text-uppercase">Correo Institucional</span>
          <div className="p-3 bg-light rounded-3 fw-medium text-dark">{item.email}</div>
        </div>
        <div className="mb-4">
          <span className="text-muted small d-block mb-1 fw-bold text-uppercase">ID del Área</span>
          <div className="p-3 bg-light rounded-3 fw-medium text-dark">{item.area_id}</div>
        </div>
        <div className="mb-4">
          <span className="text-muted small d-block mb-1 fw-bold text-uppercase">ID del Centro</span>
        ...
        <div className="d-flex justify-content-center mt-4">
          <button type="button" onClick={() => navigate('/teacher')} className="btn btn-dark px-4 py-2 fw-bold rounded-3">
            Volver al Listado
          </button>
        </div>
      </FormCard>
