...
export default function TrainingcenterShow() {
  ...
  useEffect(() => {
    setItem({
      id,
      name: 'Centro de Comercio y Servicios', location: 'Neiva', urlFoto: 'https://via.placeholder.com/600x400',
  ...
  return (
    <RequireAuth>
      <FormCard title="Información del Centro" subtitle={item.name || item.title || item.code || `Registro #${item.id}`}>
        ...
        <div className="d-flex justify-content-center mt-4">
          <button type="button" onClick={() => navigate('/trainingcenter')} className="btn btn-dark px-4 py-2 fw-bold rounded-3">
            Volver al Listado
          </button>
        </div>
      </FormCard>
